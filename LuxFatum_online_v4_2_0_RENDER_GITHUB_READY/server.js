...
const APP_META = {
  ...
  windowsPackageName: `LuxFatum-Windows-v${releaseInfo.version || packageInfo.version || "0.0.0"}.zip`
};
...
function windowsPackageFiles() {
  ...
  const nodeRuntime = path.join(ROOT, "runtime", "node.exe");
  if (fs.existsSync(nodeRuntime)) files.push({ full: nodeRuntime, rel: "runtime/node.exe", stat: fs.statSync(nodeRuntime) });
