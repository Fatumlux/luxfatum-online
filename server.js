...
const WEB_BUILD = path.join(ROOT, "web-build");
...
const RELEASE_VERSION = releaseInfo.version || packageInfo.version || "0.0.0";
const PACKAGE_VERSION_LABEL = String(RELEASE_VERSION).replace(/^v/i, "");
const MIN_SUPPORTED_VERSION = releaseInfo.minSupportedVersion || RELEASE_VERSION;
const APP_META = {
  ...
  packageName: `luxfatum-online-v${PACKAGE_VERSION_LABEL}-render-github.zip`,
  windowsPackageName: `LuxFatum-Windows-Desktop-Online-v${PACKAGE_VERSION_LABEL}.zip`,
  installerPackageName: `LuxFatum-Setup-v${PACKAGE_VERSION_LABEL}.exe`
};
...
function compareVersions(a, b) {
  const left = String(a || "0").replace(/^v/i, "").split(".").map(n => Number.parseInt(n, 10) || 0);
  const right = String(b || "0").replace(/^v/i, "").split(".").map(n => Number.parseInt(n, 10) || 0);
